...
jest.mock('axios');
jest.mock('../src/components/trainers/AnalysisModal', () => ({ assignmentId, onClose }: any) => (
  <div role="dialog">Modal for {assignmentId}<button onClick={onClose}>Close</button></div>
));
...
describe('AssignedAnalyses', () => {
  ...
  it('shows error state', () => {
    (useQuery as jest.Mock).mockReturnValue({ data: [], isLoading: false, error: true });
    render(<AssignedAnalyses />);
    expect(screen.getByText(/Ошибка загрузки данных/i)).toBeInTheDocument();
  });

  it('shows no-data message when filter yields none', () => {
    (useQuery as jest.Mock).mockReturnValue({ data: sampleData, isLoading: false, error: null });
    render(<AssignedAnalyses />);
    // enter a search term that doesn't match
    fireEvent.change(screen.getByRole('textbox'), { target: { value: 'XXX' } });
    expect(screen.getByText(/Нет данных, соответствующих запросу/i)).toBeInTheDocument();
  });

  it('renders table rows and opens modal on row click', async () => {
    (useQuery as jest.Mock).mockReturnValue({ data: sampleData, isLoading: false, error: null });
    render(<AssignedAnalyses />);
    // rows should contain analyze names
    ...
    // closing modal should call invalidateQueries
    fireEvent.click(screen.getByText('Close'));
    await waitFor(() => expect(mockInvalidate).toHaveBeenCalledWith({ queryKey: ['assignedAnalyses'] }));
  });
});
