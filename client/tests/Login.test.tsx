...
describe('Login', () => {
  ...
  beforeEach(() => {
    ...
    // Provide default useMutation shape so .isPending exists
  ...
  it('calls mutate with credentials on submit', () => {
    ...
    expect(mockMutate).toHaveBeenCalledWith({ email: 'a@b.com', password: 'secret' });
  });
  ...
  it('navigates to verify on successful login', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Войти/i }));
    await waitFor(() =>
      expect(mockNavigate).toHaveBeenCalledWith('/login-verify', { state: { email: 'x@y.com' } })
    );
  });
  ...
  it('displays error message on login failure', async () => {
    ...
    (useMutation as jest.Mock).mockImplementation(({ onError: oe }) => {
      onError = oe!;
      return { mutate: () => onError({ response: { data: { message: 'Bad creds' } } }), isPending: false };
    });
    render(<Login />);
    const emailInput = document.querySelector('input[type="email"]') as HTMLInputElement;
    const passInput = document.querySelector('input[type="password"]') as HTMLInputElement;
