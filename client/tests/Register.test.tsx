...
jest.mock("../src/components/RegisterAthlete", () => () => (
  <div>RegisterStudent Component</div>
));
jest.mock("../src/components/RegisterTrainer", () => () => (
  <div>RegisterTrainer Component</div>
));
...
describe("Register", () => {
  ...
  it('renders RegisterStudent when "Я спортсмен" selected', () => {
    mockIsAuth.mockReturnValue(false);
    render(<Register />);
    const studentRadio = screen.getByLabelText(
      /Я спортсмен/i
    ) as HTMLInputElement;
  ...
  it('renders RegisterTrainer when "Я медицинский работник" selected', () => {
    mockIsAuth.mockReturnValue(false);
    render(<Register />);
    const trainerRadio = screen.getByLabelText(
      /Я медицинский работник/i
    ) as HTMLInputElement;
