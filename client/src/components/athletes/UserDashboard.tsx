...
export default function UserDashboard() {
  ...
  const {
    ...
    queryFn: () =>
      selectedAnalysis
        ...
        : Promise.resolve({ results: [] as GroupData[] }),
  ...
  const [bounds, setBounds] = useState<
    Record<string, { lower: number | null; upper: number | null }>
  >({});
  ...
  useEffect(() => {
    ...
    const allDateTimes = new Set<string>(
      groups.flatMap((g) => g.measurements.map((m) => m.date.slice(0, 16)))
    );
    ...
    const data = sortedDateTimes.map((dt) => {
      ...
      groups.forEach((g) => {
        const match = g.measurements.find((m) => m.date.slice(0, 16) === dt);
