...
const EditStudentModal: React.FC<EditStudentModalProps> = ({
  ...
  const [selectedSport, setSelectedSport] = useState<OptionType | null>(
    student.sport_id
      ? { value: student.sport_id, label: student.sport_name! }
      : null
  );
  const {
    data: teams = [],
    ...
    queryFn: () =>
      selectedSport ? fetchTeams(selectedSport.value) : Promise.resolve([]),
    enabled: !!selectedSport,
  });
  const [selectedTeam, setSelectedTeam] = useState<OptionType | null>(
    student.team_id
      ? { value: student.team_id, label: student.team_name! }
      : null
  );
    ...
    <div className="modal-overlay">
      <div className="modal-content manage-modal">
        ...
        <div className="column">
          ...
          <Select<OptionType>
            options={
              useQuery({ queryKey: ["sports"], queryFn: fetchSports }).data ||
              []
            }
            value={selectedSport}
        ...
        <div className="edit-buttons">
          <button onClick={() => onSave(selectedSport, selectedTeam)}>
            Сохранить
          </button>
        </div>
      </div>
