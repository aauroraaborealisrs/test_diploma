...
//     if (!newSportName.trim()) return toast.warn("Введите название вида спорта!");
...
//   return (
//     <div className="teams-sports-manager">
  ...
  return (
    <div className="teams-sports-manager">
      ...
      <div className="search-pagination-wrapper">
        ...
        <RecordsPerPageSelect value={recordsPerPage} onChange={setRecordsPerPage} />
      </div>
...
const TeamsSportsManager: React.FC = () => {
  ...
  const handleUpdateStudent = async (student_id: string, field: "sport_id" | "team_id", value: string | null) => {
    try {
      ...
      setStudents((prev) =>
        prev.map((s) =>
          s.student_id === student_id ? { ...s, [field === "sport_id" ? "sport_name" : "team_name"]: value ? (field === "sport_id" ? sports.find((sp: OptionType) => sp.value === value)?.label : teams.find((tm: OptionType) => tm.value === value)?.label) : "—" } : s
        )
      );
    } catch {
