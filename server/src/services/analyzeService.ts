...
export class AnalyzeService {
  ...
  static async submitAnalysis(
    ...
    analyze_data: Record<string, any>
  ) {
  ...
  static async getDetailedResults(assignment_id: string, analyze_id: string) {
    ...
    const translatedResults = results.rows.map((result) => {
      const parameter = parametersResult.rows.find(
        (param) => param.parameter_id === result.parameter_id
      );
      return {
        ...result,
  ...
  static async getTableData(analyze_id: string) {
    ...
    results.rows.forEach((result) => {
      ...
      let assignmentResult = finalResults.find(
        (entry) => entry.assignment_id === assignment_id
      );
      if (!assignmentResult) {
        assignmentResult = {
          Имя: first_name,
