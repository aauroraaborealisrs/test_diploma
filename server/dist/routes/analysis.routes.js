...
function getTargetTable(analyzeName) {
    const tableMap = {
...
router.get('/user', async (req, res) => {
    ...
    `;
        ...
        const analysesWithStatus = await Promise.all(allAnalyses.map(async (analysis) => {
...
router.post('/submit', async (req, res) => {
    ...
    try {
        ...
        const assignmentCheck = await db.query('SELECT * FROM analyze_assignments WHERE assignment_id = $1', [assignment_id]);
        if (assignmentCheck.rowCount === 0) {
            return res.status(404).json({ message: 'Assignment not found.' });
        }
        const analyzeId = assignmentCheck.rows[0].analyze_id;
        const analyzeTypeQuery = await db.query('SELECT analyze_name FROM analyzes WHERE analyze_id = $1', [analyzeId]);
        ...
        const result = await db.query(`INSERT INTO ${targetTable} (assignment_id, student_id, analyze_id, ${Object.keys(mappedData).join(', ')}, analyze_date, created_at)
       VALUES ($1, $2, $3, ${Object.values(mappedData)
            .map((_, i) => `$${i + 4}`)
            .join(', ')}, CURRENT_DATE, CURRENT_TIMESTAMP)
       RETURNING *`, [assignment_id, student_id, analyzeId, ...Object.values(mappedData)]);
        res.status(201).json({
...
router.post('/details', async (req, res) => {
    ...
    try {
      ...
      `;
        ...
        const reversedFieldMapping = Object.fromEntries(Object.entries(fieldMapping).map(([key, value]) => [value, key]));
